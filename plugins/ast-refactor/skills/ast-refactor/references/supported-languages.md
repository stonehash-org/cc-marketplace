...
| kotlin | `*.kt`, `*.kts` | `queries/kotlin/symbols.scm` | ready |

## Adding a New Language

```bash
bash ~/.claude/plugins/local/ast-refactor/scripts/add-language.sh --lang <name>
```

Known auto-detected languages: rust, go, ruby, c, cpp, csharp, swift, php, scala

For unlisted languages, provide `--grammar-url`:

```bash
bash ~/.claude/plugins/local/ast-refactor/scripts/add-language.sh \
